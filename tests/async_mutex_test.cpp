...
TEST_CASE("scoped_lock_async (simple)") {
    TestThread thread;
    EventLog eventLog;
    avast::asio::async_mutex mutex;
    ...
    REQUIRE(eventLog.size() == 4);
    REQUIRE(eventLog[0] == Event{1, EventType::Locked});
    REQUIRE(eventLog[1] == Event{1, EventType::Unlocked});
    REQUIRE(eventLog[2] == Event{2, EventType::Locked});
    REQUIRE(eventLog[3] == Event{2, EventType::Unlocked});
}

TEST_CASE("lock_async (simple)") {
    TestThread thread;
    EventLog eventLog;
    avast::asio::async_mutex mutex;
...
TEST_CASE("lock_async (multiple coroutines)") {
    TestThread thread;
    EventLog eventLog;
    avast::asio::async_mutex mutex;
    ...
    REQUIRE(eventLog.size() == 20);
    for (int eventIdx = 0, testIdx = 0; eventIdx < eventLog.size(); eventIdx += 2, testIdx += 1) {
        REQUIRE(eventLog[eventIdx] == Event{testIdx, EventType::Locked});
        REQUIRE(eventLog[eventIdx + 1] == Event{testIdx, EventType::Unlocked});
    }
}

TEST_CASE("lock_async (multithreaded)") {
    EventLog eventLog;
    avast::asio::async_mutex mutex;

    const auto testFunc = [&eventLog, &mutex](boost::asio::io_context &ioc, int idx) -> boost::asio::awaitable<void> {
        co_await mutex.lock_async(boost::asio::use_awaitable);
        eventLog.emplace_back(idx, EventType::Locked);
